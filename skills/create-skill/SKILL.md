...
## Anti-bloat (locked policy)

Before creating a skill, check `USER_SPEECH` skill anti-bloat (#15b/#35):

1. **Skills = thin gates**, not knowledge bases. Modes → path to existing SOT → artifact → PASS/FAIL.
2. **Do not** re-host Style Bibles, best-practice essays, or long algorithm writeups inside SKILL.md. Point at paths; open on demand.
3. Prefer **AGENTS.md / README pointers** or **one project-local router** over a multi-skill pack for the same domain.
4. Prefer **Project** scope unless the workflow is truly every-repo; avoid user-global skills that tax unrelated sessions.
5. Target body **~≤200 lines** for routers. Evolve one home after the same gate fails twice — do not spawn siblings for the same job.
6. If a full SOT already exists (e.g. channel BP), create a **router** that writes packets under a known dir, not a second bible.
7. Do **not** create a skill on first mention of a workflow. Prefer documenting friction; create/extend only after the same need hits **≥2×** (USER_SPEECH #35–#36). Never invent sibling agents/plugins “to complete the stack.”