...
1. If the question is about **current config** (what MCP servers, models, or settings are active),
   read `/Users/jamie.matheson/.grok-vibe/config.toml`. MCP servers are under `[mcp_servers.*]` sections.
...
2. If the question is about **how to do something** (setup, adding MCP servers, creating skills,
   ...
   `/Users/jamie.matheson/.grok-vibe/docs/user-guide/`. The available guides are:
   ...
   `/Users/jamie.matheson/.grok-vibe/README.md` for the comprehensive reference.

3. To **modify config** for the user, edit `/Users/jamie.matheson/.grok-vibe/config.toml` with search_replace.

4. To **create a skill** for the user, create `/Users/jamie.matheson/.grok-vibe/skills/<name>/SKILL.md`
   (read `/Users/jamie.matheson/.grok-vibe/docs/user-guide/08-skills.md` for the SKILL.md format).